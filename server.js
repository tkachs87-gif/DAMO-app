...
app.post('/api/scan-contract', upload.single('contractImage'), async (req, res) => {
  try {
    ...
    마크다운 코드 블록(\`\`\`json 등)이나 설명 텍스트 없이 오직 순수 JSON 객체만 반환해줘.
    Key 목록:
    ...
    if (!response.ok) {
      console.error('Gemini API Error Response:', resultData);
      throw new Error(resultData.error?.message || 'Gemini API 호출에 실패했습니다.');
    }
